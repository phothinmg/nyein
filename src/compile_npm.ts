...
import { italic, magenta, green } from "@lwe8/tcolor";
import getConfig from "./config.js";
import { forceRemoveDir, cleanDir, wait } from "./opt/helpers.js";
 ...
 */
async function replaceExt(fileName: string, format: "esm" | "cjs") {
...
async function compileNPM(): Promise<void> {
  ...
  let cjsOutDir = "";
  let path_to_remove = "";
  let _exports: _Exports = {};
  for await (const key of exportKeys) {
    let cjsDtsFile: string[] = [];
    let cjsOutFile: string[] = [];
    let esmDtsFile: string[] = [];
    let esmOutFile: string[] = [];
