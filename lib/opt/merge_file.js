 ...
 */
export function removeImports(code, npm) {
    const importRegex = /^import[\s\S]+?from\s*['"]([^'"]+)['"]\s*;?/gm;
    const newCode = code.replace(importRegex, (match, path) => {
...
function removeExports(code) {
    ...
    code = code.replace(/^export\s+(default\s+)?(const|function|type|interface)\b/gm, "$2");
    // Remove all other export/export default statements (whole line)
    code = code.replace(/^export\s+.*;?\s*$/gm, "");
    return code.replace(/^\s*[\r\n]/gm, ""); // Remove empty lines
}
async function mergeTypescriptFile(dag) {
    const npmImport = [];
    const depsContent = [];
