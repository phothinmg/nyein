...
const commonJsAssignedRegexp = /^\s*(?:const|let|var)?\s*[\w$]*\s*=\s*require\(['"]([^'"]+)['"]\).*;?\s*$/gm;
const commonJsUnAssignedRegexp = /^\s*require\(['"]([^'"]+)['"]\).*;?\s*$/gm;
const is_commonJs = (str) => commonJsAssignedRegexp.test(str) || commonJsUnAssignedRegexp.test(str);
function checkExtGroup(exts) {
...
const checkFileExtension = async (dag) => {
    ...
    if (un_validExt.length) {
        console.error(magenta(`Unsupported file extensions ${italic(yellow(un_validExt.join(" ")))} found in dependencies tree.`));
        process.exit(1);
    }
...
const esmRegexp = /(^|\n)\s*import\s|(^|\n)\s*export\s/m;
const getModuleTypeFromDeps = async (dag) => {
    let esmCount = 0;
    let cjsCount = 0;
    for await (const file of dag) {
        const content = await fs.readFile(file, "utf8");
        if (is_commonJs(content))
            cjsCount++;
        if (esmRegexp.test(content))
            esmCount++;
    }
...
export default async function checkFiles(dag) {
    const exts = await checkFileExtension(dag);
    const moduleType = await getModuleTypeFromDeps(dag);
    if (exts === "both") {
        console.warn(italic(magenta("Both Javascript and Typescript extensions found in dependencies tree, currently unsupported.")));
        process.exit(1);
    }
    if (moduleType === "commonjs" || moduleType === "mixed") {
        console.warn(italic(magenta("Bundler found commonjs in dependencies tree,commonjs is currently unsupported.")));
        process.exit(1);
    }
}
