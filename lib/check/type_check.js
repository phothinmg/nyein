...
export default function typeCheckFromFile({ filePaths, exit = true, compileOptsOrPath = undefined, }) {
    const absoluteFilePaths = filePaths.map((fp) => path.resolve(fp));
    const compilerOptions = getCompilerOptions(compileOptsOrPath).forTypeCheck;
    // Create program
    ...
    for (const filePath of absoluteFilePaths) {
        ...
        if (diagnostics.length > 0) {
            ...
            console.error(italic(magenta(ts.formatDiagnosticsWithColorAndContext(diagnostics, formatHost))));
            console.error(italic(magenta(`TypeScript check failed in file: ${filePath}`)));
            if (exit) {
                process.exit(1);
            }
        }
